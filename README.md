...
Copy `.env.example` to `.env` for local development and provide authorized provider configuration only in the local environment. Never commit `.env`, API keys, or generated secrets. `DEEPSEEK_MODEL` remains a placeholder until the authorized provider configuration is verified in a later integration story.
